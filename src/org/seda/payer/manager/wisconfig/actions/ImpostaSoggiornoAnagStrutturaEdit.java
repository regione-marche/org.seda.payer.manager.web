...
public class ImpostaSoggiornoAnagStrutturaEdit extends BaseManagerAction implements PayerCommand{
				...
				try {
					...
					if (detailResponse != null)
					{
						if (detailResponse.getAnagraficaStruttura() != null)
						{
							...
							request.setAttribute("tx_data_da",detailResponse.getAnagraficaStruttura().getDataValiditaInizio());
							request.setAttribute("tx_data_a",detailResponse.getAnagraficaStruttura().getDataValiditaFine());
							request.setAttribute("tx_data_obbl_da",detailResponse.getAnagraficaStruttura().getDataObbligoComunicazioneInizio());
//							Calendar cal = Calendar.getInstance();
//							cal.set(2016, Calendar.SEPTEMBER, 1, 0,0,0);
//							cal.setTimeZone(TimeZone.getTimeZone("GMT"));
//							request.setAttribute("tx_data_obbl_da",cal);
							request.setAttribute("tx_data_obbl_a",detailResponse.getAnagraficaStruttura().getDataObbligoComunicazioneFine());
		...
		case TX_BUTTON_EDIT:
			...
			if (esito.equals("OK"))
			{
				...
				try {
					//Per tipologia struttura pari a Locazioni brevi / Locazioni brevi - Attività Intermediazione Immobiliare devo generare numero autorizzazione
	...
	private String checkAnagraficaStruttura (HashMap<String,Object> parametri, HttpServletRequest request)
	{
		...
		//PG180050_001 GG - inizio
		//Per tipologia struttura pari a Locazioni brevi / Locazioni brevi - Attività Intermediazione Immobiliare devo verificare che non sussista la coppia codice fiscale / p.iva e tipologia struttura
		if (isSoris(request) && (((String)parametri.get("strutturaRicettiva")).equals("018") || ((String)parametri.get("strutturaRicettiva")).equals("019"))) {
				...
				if (res!=null) {
					if (res.getResponse().getRetCode().getValue().equals("00")) {
						if (res.getAnagraficaStruttura() != null)
							return "Cod.Fisc./P.IVA " + (String)parametri.get("codiceFiscaleStruttura") + " già presente per tipologia struttura selezionata";
					} else 
