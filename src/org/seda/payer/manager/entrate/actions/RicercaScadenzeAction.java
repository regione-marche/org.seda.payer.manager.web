...
import com.seda.payer.bancadati.webservice.dati.RicercaScadenzeRequest;
import com.seda.payer.bancadati.webservice.dati.RicercaScadenzeResponse;

public class RicercaScadenzeAction extends BaseEntrateAction {
...
/*
    ...
    public void ricercaScadenze(HttpServletRequest request) throws Exception
    {
 			RicercaScadenzeResponse out;
			RicercaScadenzeRequest in;
			in = prepareRicercaScadenze(request);
						
			WSCache.logWriter.logDebug("Pagina interrogazione di test");
			out =  WSCache.entrateBDServer.ricercaScadenze(in, request);
			
			if(out.getPInfo().getNumRows() > 0) {
				request.setAttribute("listaScadenze", out.getListXml());

				request.setAttribute("listaScadenze.pageInfo", getPageInfo(out.getPInfo(), in.getPagina().getRowsPerPage()));		

			}else{
				String messaggio="";
				if (request.getAttribute("messaggioRis")!=null)
					messaggio = request.getAttribute("messaggioRis").toString() + " - ";
				setFormMessage("ricercaScadenzeForm", messaggio + Messages.NO_DATA_FOUND.format(), request);
			}
    }
    ...
    private RicercaScadenzeRequest prepareRicercaScadenze(HttpServletRequest request)
	{
    	resetPage(request);
		
    	RicercaScadenzeRequest ris;

		ris = new RicercaScadenzeRequest();
