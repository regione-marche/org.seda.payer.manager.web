...
public class MonitoraggioExtAction extends BaseMonitoraggioExtAction {
		...
		switch(firedButton) {
			case TX_BUTTON_CERCA: 
				...
				try {
					...
					RecuperaListaMIPResponse listaMIP = getListaMIP(request);
					if(listaMIP != null)
					{
						if(listaMIP.getRetCode().equals("00"))
						{
							...
							if(pageInfo != null)
							{
								if(pageInfo.getNumRows() > 0)
								{
									...
									String lista = elaboraXmlListaMip(listaMIP.getListMIPXml(), request, "monitoraggioExtForm");
							...
							setFormMessage("monitoraggioExtForm", listaMIP.getRetMessage() , request);
					}
					else 
	...
	private RecuperaListaMIPResponse getListaMIP(HttpServletRequest request) throws FaultType, RemoteException {
		...
		return WSCache.mipServer.recuperaListaMIP(listaReq, request);
	}
