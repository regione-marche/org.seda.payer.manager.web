...
public class BaseMonitoraggioExtAction extends BaseManagerAction {
	...
	public void recuperaDettaglioMIP(String chiaveTransazione, HttpServletRequest request)
	{
		RecuperaListaMIPRequest listaReq = new RecuperaListaMIPRequest();
		...
		try {
			RecuperaListaMIPResponse res = WSCache.mipServer.recuperaListaMIP(listaReq, request);
			
			if (res != null)
			{
				String lista = elaboraXmlListaMip(res.getListMIPXml(), request, "frmMonitoraggioExtDetails");
	...
	{
		RecuperaListaMPSRequest listaReq = new RecuperaListaMPSRequest();
		...
		try {
			RecuperaListaMPSResponse listaMPS = WSCache.mipServer.recuperaListaMPS(listaReq, request);
			...
			if(listaMPS != null)
			{
				if(listaMPS.getRetCode().equals("00"))
				{
					...
					if(pageInfo != null)
					{
						...
						{
							String lista = elaboraXmlListaMps(listaMPS.getListMPSXml(), request, "frmMonitoraggioExtDetails");
					...
					setFormMessage("frmMonitoraggioExtDetails", listaMPS.getRetMessage() , request);
			}
			else 
