...
public class RicercaRiversamentiDettContrAction extends BaseRiversamentoAction {
		...
		switch(getFiredButton(request)) 
			...
			case TX_BUTTON_CERCA: 
				...
				catch (FaultType fte) 
				{
					...
					fte.printStackTrace();
					setFormMessage("ricercaRiversamentiDettContrForm", "errore: " + decodeMessaggio(fte), request);
				}
				catch (RemoteException af)
				{
					WSCache.logWriter.logError("errore: " + af.getMessage(),af);
					af.printStackTrace();
					setFormMessage("ricercaRiversamentiDettContrForm", testoErroreColl, request);					
				}
				catch (Exception e) 
				   {
					WSCache.logWriter.logError("errore: " + e.getMessage(),e);
					//setErrorMessage(request, e.getMessage());
					setFormMessage("ricercaRiversamentiDettContrForm", "errore: " + testoErrore, request);
					}
				break;
			}
...
//		requestData(request, "c17", "c17S");
    ...
    public void ricercaDettagliRiv(HttpServletRequest request, String tipoDettaglio) throws Exception
    {
			...
			{
				request.setAttribute("listaDettaglioContr", out.getListXml());
				request.setAttribute("listaDettaglioContr.pageInfo", getPageInfo(out.getPInfo(), in.getPagina().getRowsPerPage()));				
...
//			else setMessage("Nessun risultato");
			else setFormMessage("ricercaRiversamentiDettContrForm", Messages.NO_DATA_FOUND.format(), request);					
