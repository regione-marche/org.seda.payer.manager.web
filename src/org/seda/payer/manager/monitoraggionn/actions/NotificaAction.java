...
public class NotificaAction extends BaseManagerAction {
	...
	private boolean initMPS(String idTransazione, String numeroOperazione, String idPortale, String formName, HttpServletRequest request, HttpSession session)
	{
		...
		ModuloIntegrazionePagamentiNodoPaymentStatus mipPS = new ModuloIntegrazionePagamentiNodoPaymentStatus();
		...
		MINSavePaymentStatusRequest reqPS = new MINSavePaymentStatusRequest(mipPS);
		try {
			MINSavePaymentStatusResponse resPS = WSCache.mipServer.savePaymentNodoStatus(reqPS, request);
