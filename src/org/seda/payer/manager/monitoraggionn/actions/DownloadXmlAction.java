...
//						ModuloIntegrazionePagamenti mip = getMIP(idTransazione, nomeForm, request);
						ModuloIntegrazionePagamentiNodo mip = getMIP(idTransazione, nomeForm, request);
						if (mip != null)
						{
...
//							YLM PG22XX07 INI
							else if (type.equals("xmlsendrt"))
								sXml = mip.getXmlSendRT();
//							YLM PG22XX07 FINE
						}
					}
					else if (source.equalsIgnoreCase("mps"))
					{
						if (request.getAttribute("gruppo") != null && request.getAttribute("progr") != null)
						{
							...
							ModuloIntegrazionePagamentiNodoPaymentStatus mps = getMPN(idTransazione, iGruppo, iProgr, nomeForm, request);
							if (mps != null)
							{
...
//	}
//marini
	private ModuloIntegrazionePagamentiNodo getMIP(String idTransazione, String formName, HttpServletRequest request)
	{
		MINDetailRequest req = new MINDetailRequest();
		req.setChiaveTransazione(idTransazione);
		
		try {
			MINDetailResponse res = WSCache.mipServer.getModuloIntegrazionePagamentiNodo(req, request);
			
			if (res != null)
				return res.getMip();
			else
				setFormMessage(formName, "Impossibile recuperare i dati MIN richiesti.", request);
		} 
		catch (Exception e) {
			//setErrorMessage(e.getMessage());
			e.printStackTrace();
	...
	private ModuloIntegrazionePagamentiNodoPaymentStatus getMPN(String idTransazione, int gruppo, int progressivo,
			String formName, HttpServletRequest request)
	{
		MPNDetailRequest req = new MPNDetailRequest();
		req.setChiaveTransazione(idTransazione);
		req.setGruppoTentativiNotifica(gruppo);
		req.setNumeroTentativoNotifica(progressivo);
		
		try {
			MPNDetailResponse res = WSCache.mipServer.getModuloIntegrazionePagamentiNodoPaymentStatus(req, request);
			
			if (res != null)
				return res.getMipStatus();
			else
				setFormMessage(formName, "Impossibile recuperare i dati MPN richiesti.", request);
		} 
		catch (Exception e) {
			//setErrorMessage(e.getMessage());
			e.printStackTrace();
