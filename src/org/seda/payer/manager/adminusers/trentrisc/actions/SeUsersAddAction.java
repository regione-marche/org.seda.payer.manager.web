package org.seda.payer.manager.adminusers.trentrisc.actions;
...
import com.seda.payer.pgec.webservice.commons.dati.GetKeyRequest;
import com.seda.payer.pgec.webservice.commons.dati.GetKeyResponse;
...
public class SeUsersAddAction extends BaseAdminusersAction implements PayerCommand{
	...
	public Object execute(HttpServletRequest request) throws PayerCommandException {
		...
		switch(firedButton)
		{
			...
			case TX_BUTTON_NUOVO: 
				...
				profilo.setUsername(getUsername(request));
	...
	private String getUsername (HttpServletRequest request){
		...
		GetKeyRequest req=new GetKeyRequest("999");

		try {
			GetKeyResponse res = WSCache.commonsServer.getKey(req, request);
			username="ITTR" + Pad.left(res.getChiave(), 8, '0') ;
		} catch (FaultType e) {
			e.printStackTrace();
