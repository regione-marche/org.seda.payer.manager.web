...
public class RicercaArticoliAction extends BaseRuoliAction {
    ...
    public void ricercaArticoli(HttpServletRequest request) throws Exception
    {
    	
			RicercaArticoliResponse out;
			RicercaArticoliRequest in;
			in = prepareRicerca(request);
						
			WSCache.logWriter.logDebug("Pagina interrogazione di test");
			out =  WSCache.ruoliBDServer.ricercaArticoli(in, request);
			
			if(out.getPInfo().getNumRows() > 0) 
			{
				request.setAttribute("listaArticoli", out.getListXml());

				request.setAttribute("listaArticoli.pageInfo", getPageInfo(out.getPInfo(), in.getPagina().getRowsPerPage()));		
				...
				request.setAttribute("totImportoTotaleMora", out.getTotMora().toString());
			...
			else
			{
				...
				setFormMessage("ricercaArticoliForm", messaggio + Messages.NO_DATA_FOUND.format(), request);
			}
	...
	private RicercaArticoliRequest prepareRicerca(HttpServletRequest request)
	{
		PropertiesTree configuration; 
		RicercaArticoliRequest ris;
...
//      if (request.getParameter("rowsPerPage")!=null)	
		...
		ris = new RicercaArticoliRequest();
