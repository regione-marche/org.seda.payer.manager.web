...
<script>
	...
	<c:url value="" var="formParameters">
		...
		<c:if test="${!empty param.codSoc}">  
			<c:param name="codSoc">${param.codSoc}</c:param>
		</c:if>
		<c:if test="${!empty param.codUte}">  
			<c:param name="codUte">${codUte}</c:param>
		</c:if>
		<c:if test="${!empty param.dataRiv}">  
			<c:param name="dataRiv">${param.dataRiv}</c:param>
		</c:if>
		<c:if test="${!empty param.codEnte}">  
			<c:param name="codEnte">${param.codEnte}</c:param>
		</c:if>
		<c:if test="${!empty param.tipoRend}">  
			<c:param name="tipoRend">${param.tipoRend}</c:param>
		</c:if>
		<c:if test="${!empty param.tipoRiv}">  
			<c:param name="tipoRiv">${param.tipoRiv}</c:param>
		</c:if>
		<c:if test="${!empty param.stato}">  
			<c:param name="stato">${param.stato}</c:param>
		</c:if>
<!-- parametri alert flag CBI -->		
		<c:if test="${!empty param.fcbi}">  
			<c:param name="fcbi">${param.fcbi}</c:param>
		</c:if>
		<c:if test="${!empty param.coord}">  
			<c:param name="coord">${param.coord}</c:param>
		</c:if>
	...
	<c:if test="${tx_button_set_flag_cbi!=null}">
		<s:form name="ConfermaFlagForm" action="aggiornaFlagCBI.do${formParameters}" method="post" hasbtn1="false" hasbtn2="false" hasbtn3="false">	
			<s:table  border="0" cellspacing="0" cellpadding="3" cssclass="seda-ui-datagrid">
				<s:thead>
					<s:tr cssclass="seda-ui-datagridrowpari">
						<s:th cssclass="seda-ui-datagridheadercell"><b>Avanzamento</b></s:th>
					</s:tr>
				</s:thead>
				<s:tbody>
				    <s:tr>
				    	<s:td cssclass="seda-ui-datagridcell textcenter" >
							<s:label name="avviso" text="Il riversamento ha la 'Data Riversamento' superiore alla data odierna. Continuare?" cssclass="textred" />	
						</s:td>
					</s:tr>			
				</s:tbody>
			</s:table>
			<s:div name="divRicercaBottoni" cssclass="divRicBottoni">
	   			<s:textbox name="avanzaFlag" label="avanzaFlag" bmodify="true" text="" cssclass="display_none" cssclasslabel="display_none"  />   			
		   		<s:button id="tx_button_set_flag_cbi" cssclass="btnStyle" type="submit" text="Si" onclick=""/>
	   			<s:button id="tx_button_cerca" cssclass="btnStyle" type="submit" text="No" onclick=""/>
			</s:div>
		</s:form>
	</c:if>

	<c:if test="${tx_button_avanza_stato!=null}">
		<s:form name="ConfermaStatoForm" action="aggiornaStatoRiv.do${formParameters}" method="post" hasbtn1="false" hasbtn2="false" hasbtn3="false">
			...
			<s:div name="divRicercaBottoni" cssclass="divRicBottoni">
	   			<s:textbox name="avanzaStato" label="avanzaStato" bmodify="true" text="" cssclass="display_none" cssclasslabel="display_none"  />   			
	   			<s:button id="tx_button_avanza_stato" cssclass="btnStyle" type="submit" text="Si" onclick=""/>
	   			<s:button id="tx_button_cerca" cssclass="btnStyle" type="submit" text="No" onclick=""/>
			</s:div>
