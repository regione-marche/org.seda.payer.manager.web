...
<c:if test="${!empty listaDettaglioContr}">
	...
	<s:datagrid  viewstate="" cachedrowset="listaDettaglioContr" action="riversamentiDettaglioContr.do" border="1" usexml="true"  rowperpage="${applicationScope.rowsPerPage}">
		<s:action>
			<c:url value="riversamentiDettaglioContr.do">				
				<c:if test="${!empty param.rRowsPerPage}">
...
<!-- 
DDC_CTDTCFIS, ANE_CANECENT, ANE_TANETUFF, ANE_CANECUFF, 
       RED_CTSECTSE, RED_CREDANNO, 
       CAN_DCANDCAN, GTW_DGTWDGTW, 
       RED_GREDPAGA, 
       SUM(BOL_IBOLTOTA) AS TOT_IBOLTOTA, 
       SUM(BOL_IBOLCONC) AS TOT_IBOLCONC, 
       SUM(BOL_IBOLCGTW) AS TOT_IBOLCGTW, 
       SUM(BOL_IBOLSPES) AS TOT_IBOLSPES, 
       SUM(BOL_IBOLGESC) AS TOT_IBOLGESC, 
       (CASE WHEN REV_FREVTIPO = 'R' THEN SUM(BOL_IBOLIREV) ELSE 0 END) AS TOT_IBOLIREV_R, 
       (CASE WHEN REV_FREVTIPO = 'C' THEN SUM(BOL_IBOLIREV) ELSE 0 END) AS TOT_IBOLIREV_C 
 -->

        <!--  DDC_CTDTCFIS -->
		<s:dgcolumn label="Codice Fiscale" asc="CFISA" desc="CFISD">		    

				<s:hyperlink
					href="#"
					onclick="showDettaglio('Dettaglio Transazione', 'Chiave transazione','{17}','','');return false"
  	    ...
  	    <!-- ANE_CANECENT -->
		<s:dgcolumn index="2" label="Ente Impositore" asc="CENTA" desc="CENTD"></s:dgcolumn>
        ...
        <!--  RED_CTSECTSE -->
		<s:dgcolumn index="5" label="Tipo Serv." asc="CTSEA" desc="CTSED"></s:dgcolumn>

		<!-- RED_CREDANNO -->
		<s:dgcolumn index="6" label="Anno Riferim." asc="ANNOA" desc="ANNOD" css="text_align_right"></s:dgcolumn>

		<!-- CAN_DCANDCAN -->
		<s:dgcolumn index="7" label="Canale" asc="DCANA" desc="DCAND"></s:dgcolumn>

		<!-- GTW_DGTWDGTW -->
		<s:dgcolumn index="8" label="Tipo Carta" asc="DGTWA" desc="DGTWD"></s:dgcolumn>

		<!-- RED_GREDPAGA -->
		<s:dgcolumn index="9" format="dd/MM/yyyy" label="Data Pagamento" asc="PAGAA" desc="PAGAD"></s:dgcolumn>

		<!-- RED_IREDTOTA -->
		<s:dgcolumn index="10" format="#,##0.00" label="Importo" asc="TOTAA" desc="TOTAD" css="text_align_right"></s:dgcolumn>

		<!-- RED_IREDCONC -->
		<s:dgcolumn index="11" format="#,##0.00" label="Contributo cittadino" asc="CONCA" desc="CONCD" css="text_align_right"></s:dgcolumn>

		<!-- RED_IREDCGTW -->
		<s:dgcolumn index="12" format="#,##0.00" label="Commissioni Banca" asc="CGTWA" desc="CGTWD" css="text_align_right"></s:dgcolumn>

		<!-- RED_IREDSPES -->
		<s:dgcolumn index="13" format="#,##0.00" label="Spese Notifica" asc="SPESA" desc="SPESD" css="text_align_right"></s:dgcolumn>

		<!-- RED_IREDGESC -->
		<s:dgcolumn index="14" format="#,##0.00" label="Commissioni gestore" asc="GESCA" desc="GESCD" css="text_align_right"></s:dgcolumn>

		<!-- RED_IREDIREV_R -->
		<s:dgcolumn index="15" format="#,##0.00" label="Importo Rive. Benef." asc="IREVRA" desc="IREVRD" css="text_align_right"></s:dgcolumn>

		<!-- RED_IREDIREV_C -->
		<s:dgcolumn index="16" format="#,##0.00" label="Importo Recupero" asc="IREVCA" desc="IREVCD" css="text_align_right"></s:dgcolumn>
	...
	<s:table  border="1" cellspacing="0" cellpadding="3"  cssclass="seda-ui-datagrid">
		...
		<s:tbody>
			...
			<s:tr cssclass="seda-ui-datagridrowpari">
					<s:td cssclass="seda-ui-datagridcell text_align_right">${NumTransazioni}</s:td>
	...
	<s:div cssclass="seda-ui-div" name="popupDettaglio">
		<s:div name="div_table_trans">
			<s:table border="1" cellspacing="0" cssclass="seda-ui-datagrid tableDettaglio1">
				...
				<s:tbody>
					<s:tr>
						<s:td cssclass="seda-ui-cellheader" id="nomeCampo">Nome Campo:</s:td>
						<s:td id="valoreCampo" icol="3">Valore Campo</s:td>
					</s:tr>
				</s:tbody>
			</s:table>
		</s:div>
