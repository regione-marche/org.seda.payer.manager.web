...
<c:if test="${codop == 'add'}">
	...
	$(document).ready(function(){
		loadAutoCompletion("tx_codiceFiscale");
	});
	</script>
</c:if>
					...
					<s:div name="divElement4" cssclass="divRicMetadatiSingleRow">
						<c:if test="${codop == 'add'}">
							<s:textbox bmodify="true" name="tx_codiceFiscale"
								label="UserId(CF):" maxlenght="16" showrequired="true" 
								cssclasslabel="label65 bold textright"
								cssclass="textareaman" validator="required;accept=^[a-zA-Z]{6}[\d]{2}[a-zA-Z][\d]{2}[a-zA-Z][\d]{3}[a-zA-Z]|^[0-9]{11}$"
						...
						<c:if test="${codop == 'edit'}">
							<s:textbox bmodify="${codiceFiscaleModificabile}" name="tx_codiceFiscale"
								label="UserId(CF):" maxlenght="16" showrequired="true" 
								cssclasslabel="label65 bold textright"
								cssclass="textareaman border_none" validator="required;accept=^[a-zA-Z]{6}[\d]{2}[a-zA-Z][\d]{2}[a-zA-Z][\d]{3}[a-zA-Z]|^[0-9]{11}$"
								message="[accept=UserId (CF): ${msg_configurazione_codicefiscale_piva}]"
								text="${tx_codiceFiscale}" />
						</c:if>
				...
				<s:div name="divRicercaLeft" cssclass="divRicMetadatiLeft">
					...
					<c:choose>
						<c:when test="${tx_userprofile == 'PYCO' || tx_userprofile == 'OPER'}">
							<%--Per il profilo OPER non è prevista la gestione del multiutente --%>
