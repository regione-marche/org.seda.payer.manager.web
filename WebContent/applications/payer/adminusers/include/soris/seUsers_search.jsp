			...
			<s:div name="divRicMetadati" cssclass="divRicMetadati">
				...
				<s:div name="divRicercaLeft" cssclass="divRicMetadatiLeft">
					...
					<s:div name="divElement41" cssclass="divRicMetadatiSingleRow">
						<s:textbox bmodify="true" name="tx_email"
							label="E-mail Utente:" maxlenght="50" 
							cssclasslabel="label85 bold textright"
							cssclass="textareaman" 
							text="${tx_email}" />
					</s:div>
					...
					<s:div name="divElement42" cssclass="divRicMetadatiSingleRow">
						<s:textbox bmodify="true" name="tx_ragionesociale"
							label="Rag. Soc.:" maxlenght="256" 
							cssclasslabel="label85 bold textright"
							cssclass="textareaman" 
							text="${tx_ragionesociale}" />
					</s:div>
				...
				<s:div name="divRicercaCenter" cssclass="divRicMetadatiCenter">
					...
					<s:div name="divElement91" cssclass="divRicMetadatiSingleRow">
						<s:textbox bmodify="true" name="tx_cognomenome"
							label="Cognome e/o Nome:" maxlenght="306" 
							cssclasslabel="label85 bold textright"
							cssclass="textareaman" 
							text="${tx_cognomenome}" />
					</s:div>
					...
					<s:div name="divElement92" cssclass="divRicMetadatiSingleRow">
						<s:textbox bmodify="true" name="tx_numautorizzazione"
							label="N° Autoriz. Imp. Sogg.:" maxlenght="256" 
							cssclasslabel="label85 bold textright"
							cssclass="textareaman" 
							text="${tx_numautorizzazione}" />
					</s:div>
				...
				<s:div name="divRicercaRight" cssclass="divRicMetadatiRight">
					...
					<s:div name="divElement9" cssclass="divRicMetadatiSingleRow">
						<s:dropdownlist name="tx_tipoutenza" disable="false"
							cssclass="tbddlMax floatleft" label="Tipo Persona:"
							cssclasslabel="label85 bold textright"
							valueselected="${tx_tipoutenza}">
							<s:ddloption text="Tutte le Persone" value="" />
							<s:ddloption text="Persone Fisiche" value="F" />
							<s:ddloption text="Persone Giuridiche" value="G" />
							<s:ddloption text="Ditte Individuali" value="D" />
						</s:dropdownlist>
					</s:div>					
...
<c:if test="${!empty listaUsers}">
	...
	<s:div name="div_datagrid">
		...
		<%-- USERID --%> 
			<s:dgcolumn label="UserId" asc="SE_USERID_A" desc="SE_USERID_D" >
		...
		<%-- UTENTE ATTIVO/INATTIVO --%>
			...
			<%-- PASSWORD SCADUTA --%>
			<s:dgcolumn label="Password<br/>Scaduta" asc="SE_SCAD_A" desc="SE_SCAD_D" css="textcenter">
				<%-- NON SCADUTA --%>
				<s:if right="{19}" control="eq" left="N" >
					<s:then>
						<s:image height="16" width="16" alt="User {5} attivo" 
							src="../applications/templates/adminusers/img/circle_green.png"/>
					</s:then>
				...
				<%-- SCADUTA --%>
				<s:if right="{19}" control="eq" left="Y" >
					<s:then>
						<s:image height="16" width="16" alt="User {5} non attivo" 
							src="../applications/templates/adminusers/img/circle_red.png"/>
					</s:then>
		...
		<%-- SBLOCCO UTENTE --%>
				...
				<s:if right="{20}" control="eq" left="1">
					<s:then>
						<s:hyperlink
							cssclass="hlStyle" 
		...
		<%-- RESET PASSWORD --%>
				<s:hyperlink
					cssclass="hlStyle" 
					href="seUsersResetPswd.do?tx_username={5}"
					imagesrc="../applications/templates/shared/img/Lock.png"
					alt="Reset Password User {5}" text="" />
		<%-- REINVIO PUK --%>
				<s:hyperlink
					cssclass="hlStyle" 
					href="seUsersReinvioPUK.do?tx_username={5}&vista=seusers_search"
					imagesrc="../applications/templates/adminusers/img/invioPuk.png"
					alt="Reinvio PUK User {5}" text="" />
		<%-- REINVIO MODULO AUTORIZZAZIONE --%>
				<%--<s:if right="{11}" control="eq" left="G">
					<s:then> --%>
						<!-- PG170280 CT - Rimosso link invio modulo autorizzazione -->
						<%--<s:hyperlink
							cssclass="hlStyle" 
							href="seUsersReinvioModuloAutorizzazione.do?tx_username={5}&vista=seusers_search&CN={6}&CR={7}&CD={8}&CSL={9}&CSO={10}"
							imagesrc="../applications/templates/adminusers/img/invioAutorizzazione.png"
							alt="Reinvio Modulo Autorizzazione User {5}" text="" />--%>
				<%--</s:then>
				</s:if> 
				--%>
