...
<script type="text/javascript">
	...
	$(function() {
		$.datepicker.setDefaults($.datepicker.regional["it"]);
		$("#tx_data_accr_da_hidden").datepicker("option", "dateFormat", "dd/mm/yyyy");
		$("#tx_data_accr_da_hidden").datepicker( {
			minDate: new Date(annoDa, 0, 1),
			maxDate: new Date(annoA, 11, 31),
			yearRange: annoDa + ":" + annoA,
			showOn : "button",
			...
			buttonImageOnly : true,
			onSelect : function(dateText, inst) {
				$("#tx_data_accr_da_day_id").val(dateText.substr(0, 2));
				$("#tx_data_accr_da_month_id").val(dateText.substr(3, 2));
				$("#tx_data_accr_da_year_id").val(dateText.substr(6, 4));
			},
			beforeShow: function(input, inst) {
				//imposta il valore del calendario in base a quanto impostato nelle 3 dropdownlist
	            updateValoreDatePickerFromDdl("tx_data_accr_da_day_id",
				                            "tx_data_accr_da_month_id",
				                            "tx_data_accr_da_year_id",
				                            "tx_data_accr_da_hidden");
			}
		});
		$("#tx_data_accr_a_hidden").datepicker("option", "dateFormat", "dd/mm/yyyy");
		$("#tx_data_accr_a_hidden").datepicker( {
			minDate: new Date(annoDa, 0, 1),
			maxDate: new Date(annoA, 11, 31),
			yearRange: annoDa + ":" + annoA,
			showOn : "button",
			...
			buttonImageOnly : true,
			onSelect : function(dateText, inst) {
				$("#tx_data_accr_a_day_id").val(dateText.substr(0, 2));
				$("#tx_data_accr_a_month_id").val(dateText.substr(3, 2));
				$("#tx_data_accr_a_year_id").val(dateText.substr(6, 4));
			},
			beforeShow: function(input, inst) {
				//imposta il valore del calendario in base a quanto impostato nelle 3 dropdownlist
	            updateValoreDatePickerFromDdl("tx_data_accr_a_day_id",
				                            "tx_data_accr_a_month_id",
				                            "tx_data_accr_a_year_id",
				                            "tx_data_accr_a_hidden");
			}
		});
	});
</script>
...
<!-- fine mod mp -->
			...
			<s:div name="divRicMetadati" cssclass="divRicMetadati">
				...
				<s:div name="divRicercaCenter" cssclass="divRicMetadatiCenter">
					...
					<s:div name="divElement12" cssclass="divRicMetadatiSingleRow">
						<s:dropdownlist name="tx_mostra" disable="false" cssclass="tbddl"
							...
							<s:ddloption value="2" text="Transazioni Fallite od Annullate" />
							<s:ddloption value="0" text="Transazioni Sospese" />
							<s:ddloption value="6" text="Transazioni con richiesta di storno" />
							<s:ddloption value="7" text="Transazioni Stornate" />
						</s:dropdownlist>
					</s:div>
					...
					<s:div name="divElement14c" cssclass="divRicMetadatiSingleRow">
							<s:dropdownlist name="tx_recuperate" disable="false"
								...
								cssclasslabel="label85 bold textright" label="Transaz. Recuperate:"
								valueselected="${tx_recuperate}">
								<s:ddloption value="" text="Tutte" />
				...
				<s:div name="divRicercaRight" cssclass="divRicMetadatiRight">
					...
					<s:div name="divElement180" cssclass="divRicMetadatiDoubleRow">
						...
						<s:div name="divElement180b" cssclass="floatleft">
							<s:div name="divDataAccrDa" cssclass="divDataDa">
								<s:date label="Da:" prefix="tx_data_accr_da"
									yearbegin="${ddlDateAnnoDa}" yearend="${ddlDateAnnoA}"
									locale="IT-it" descriptivemonth="false" separator="/"
									calendar="${tx_data_accr_da}" cssclasslabel="labelsmall"
									cssclass="dateman">
								</s:date>
								<input type="hidden" id="tx_data_accr_da_hidden" value="" />
							</s:div>

							<s:div name="divDataAccrA" cssclass="divDataA">
								<s:date label="A:" prefix="tx_data_accr_a"
									yearbegin="${ddlDateAnnoDa}" yearend="${ddlDateAnnoA}"
									locale="IT-it" descriptivemonth="false" separator="/"
									calendar="${tx_data_accr_a}" cssclasslabel="labelsmall"
									cssclass="dateman">
								</s:date>
								<input type="hidden" id="tx_data_accr_a_hidden" value="" />
							</s:div>
						</s:div>
					...
					<s:div name="divElement19" cssclass="divRicMetadatiSingleRow">
						...
						<c:if test="${requestScope.isRiconciliazioneAct}">
							<s:dropdownlist name="tx_riconciliata" disable="false"
								...
								<s:ddloption value="PZ" text="Parzialmente" />
							</s:dropdownlist>
						</c:if>
					</s:div>
	...
	<s:div name="divRisultati" cssclass="divRisultati">
		<s:datagrid cachedrowset="tx_lista_transazioni"
			...
			<s:dgcolumn label="Annullo" asc="SATA" desc="SATD">
				<s:if right="{30}" control="eq" left="0">
					<s:then>No</s:then>
				</s:if>
				<s:if right="{30}" control="eq" left="1">
					<s:then>Si</s:then>
				</s:if>
				<s:if right="{30}" control="eq" left="2">
			...
			<s:ifdatagrid left="${requestScope.isRiconciliazioneAct}"
				...
				<s:thendatagrid>
					<s:dgcolumn label="Riconciliata" asc="SRID" desc="SRID">
						...
						<s:if right="{12}" control="eq" left="PZ" >
						<s:then>
							Parzialmente
							</s:then>
						</s:if>
			...
			<%-- inizio LP PG200360 --%>
			<s:dgcolumn index="31" label="Recuperata" asc="RECA" desc="RECD" />
			<%-- fine LP PG200360 --%>
			...
			<s:dgcolumn label="Stato" css="textcenter">
				...
				<s:if right="{15}{25}" control="eq" left="10">
					<s:then>
						<s:image
				...
				<s:if right="{15}{25}" control="eq" left="11">
					<s:then>
						<s:image
							src="../applications/templates/monitoraggio/img/circle_yellow.png"
							alt="Storno in corso" height="16" width="16"
							cssclass="hlStyle" />
					</s:then>
				</s:if>		
				<s:if right="{15}{25}" control="eq" left="20">
					<s:then>
						<s:image
							src="../applications/templates/monitoraggio/img/circle_red.png"
							alt="Transazione fallita od annullata" height="16" width="16"
							cssclass="hlStyle" />
					</s:then>
				</s:if>
				<s:if right="{15}{25}" control="eq" left="22">
					<s:then>
						<s:image
							src="../applications/templates/monitoraggio/img/circle_black.png"
							alt="Storno confermato" height="16" width="16"
							cssclass="hlStyle" />
					</s:then>
				</s:if>
			</s:dgcolumn>
			...
			<!-- inizio LP PG190220                                            -->
			<!--  TODO: Quali azioni bloccare per stato annullo {30} = 0, 1, 2 -->
			<!--        per adesso se stato annullo == 2 non consento azioni    -->
			...
			<s:dgcolumn
				label="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Azioni&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;">
				<!-- stato annullo 0 -->
				<s:if right="{15}{21}{30}" control="eq" left="1Y0" operator="or"
					secondright="{15}{21}{30}" secondcontrol="eq" secondleft="1N0">
					<s:then>
				...
				<s:if right="{15}{21}{30}" control="eq" left="1X0">
					<s:then>
						<c:if
							test="${sessionScope.j_user_bean.azioniPerTransazioniOkEnabled}">
							<s:hyperlink
						...
						<c:if
							test="${sessionScope.j_user_bean.azioniPerTransazioniOkEnabled && userProfile!='AMEN'}">
							<s:hyperlink
								href="stampaPDFBollettini.do?tx_codice_transazione_hidden={1}"
								imagesrc="../applications/templates/monitoraggio/img/stampaBollettini.png"
								alt="Stampa Allegati PDF" text=""
				...
				<s:if right="{15}{30}" control="eq" left="00">
					<s:then>
						<c:if
				...
				<s:if right="{12}{15}{20}{25}{26}{30}" control="eq" left="NO1I0Y0">  	
					<s:then>	
						<c:if test="${sessionScope.j_user_bean.azioniPerTransazioniKoEnabled}">																			
							<s:hyperlink href="richiediStorno.do?tx_codice_transazione_hidden={1}&tx_canale_pagamento={19}&vista=monitoraggiotransazioni"
								imagesrc="../applications/templates/monitoraggio/img/ico_storno2.png" 
								alt="Esegui Storno" text=""
								cssclass="hlStyle" />
						
						</c:if>
					</s:then>
				</s:if>
				<s:if right="{15}{20}{30}" control="eq" left="0M0">
					<s:then>
						<c:if
				...
				<!-- stato annullo 1 -->
				<s:if right="{15}{21}{30}" control="eq" left="1Y1" operator="or"
					secondright="{15}{21}{30}" secondcontrol="eq" secondleft="1N1">
					<s:then>
						<c:if test="${sessionScope.j_user_bean.azioniPerTransazioniOkEnabled}">
							<s:hyperlink
								href="inviaNotificaContribuente.do?tx_codice_transazione_hidden={1}&vista=monitoraggiotransazioni"
				...
				<s:if right="{15}{21}{30}" control="eq" left="1X1">
					<s:then>
						<c:if
							test="${sessionScope.j_user_bean.azioniPerTransazioniOkEnabled}">
							<s:hyperlink
				...
				<s:if right="{15}{30}" control="eq" left="01">
					<s:then>
						<c:if
							test="${sessionScope.j_user_bean.azioniPerTransazioniKoEnabled}">
							<s:hyperlink
				...
				<s:if right="{12}{15}{20}{25}{26}{30}" control="eq" left="NO1I0Y1">  	
					<s:then>	
						<c:if test="${sessionScope.j_user_bean.azioniPerTransazioniKoEnabled}">																			
							<s:hyperlink href="richiediStorno.do?tx_codice_transazione_hidden={1}&tx_canale_pagamento={19}&vista=monitoraggiotransazioni"
								imagesrc="../applications/templates/monitoraggio/img/ico_storno2.png" 
				...
				<s:if right="{15}{20}{30}" control="eq" left="0M1">
					<s:then>
						<c:if
							test="${requestScope.isRiconciliazioneAct}">
							<s:hyperlink
				...
				<!-- non riconciliata - stato successo - agganciata a flusso fittizio -->
				<%--
				<s:if right="{12}{15}{24}" control="eq" left="NO1N">
					<s:then>
						<c:if test="${sessionScope.j_user_bean.azioniPerTransazioniOkEnabled and (sessionScope.j_user_bean.userProfile == 'AMMI' or sessionScope.j_user_bean.userProfile == 'AMSO')}">
							
							<s:hyperlink 
								href="richiediRiconciliazione.do?tx_codice_transazione_hidden={1}&vista=monitoraggiotransazioni"
								imagesrc="../applications/templates/monitoraggio/img/aggiungi.png" 
								alt="Riconcilia con flusso di quadratura fittizio" text=""
								cssclass="hlStyle" />
