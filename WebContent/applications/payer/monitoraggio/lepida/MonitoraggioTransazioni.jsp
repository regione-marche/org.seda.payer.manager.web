				...
				<s:div name="divRicercaCenter" cssclass="divRicMetadatiCenter">
					...
					<s:div name="divElement14b" cssclass="divRicMetadatiSingleRow">
						<s:textbox validator="ignore;digits;maxlength=20" bmodify="true"
							...
							text="${tx_chiave_quadratura}" maxlenght="20"
							cssclass="textareaman" cssclasslabel="label85 bold textright" />
					</s:div>
				</s:div>
...
<c:if test="${!empty tx_lista_transazioni}">
	...
	<s:div name="divRisultati" cssclass="divRisultati">
		<s:datagrid cachedrowset="tx_lista_transazioni"
			...
			<s:dgcolumn
				label="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Azioni&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;">
				<s:if right="{15}{21}" control="eq" left="1Y" operator="or"
					secondright="{15}{21}" secondcontrol="eq" secondleft="1N">
					<s:then>
				...
				<s:if right="{15}{21}" control="eq" left="1X">
					<s:then>
						<c:if
				...
				<s:if right="{15}" control="eq" left="0">
					<s:then>
						<c:if
							...
							<s:hyperlink
								href="eliminaTransazione.do?tx_codice_transazione_hidden={1}&vista=monitoraggiotransazioni&tx_button_delete=tx_button_delete"
								imagesrc="../applications/templates/monitoraggio/img/elimina.png"
								alt="Elimina" text="" cssclass="hlStyle hlChannel{19}Style" />
						</c:if>
					</s:then>
					<s:else></s:else>
				</s:if>
				<s:if right="{12}{15}{20}{25}{26}" control="eq" left="NO1I0Y">  	
					<s:then>	
				...
				<s:if right="{15}{20}" control="eq" left="0M">
					<s:then>
						<c:if
				...
				<s:if right="{12}{15}{24}" control="eq" left="NO1N">
					<s:then>
						<c:if test="${sessionScope.j_user_bean.azioniPerTransazioniOkEnabled and (sessionScope.j_user_bean.userProfile == 'AMMI' or sessionScope.j_user_bean.userProfile == 'AMSO')}">
							<!--
							<s:hyperlink 
								href="assegnaTranAFittizio.do?tx_codice_transazione_hidden={1}&vista=monitoraggiotransazioni"
