...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#tx_data_conferma_da_hidden").datepicker("option", "dateFormat", "dd/mm/yyyy");
		$("#tx_data_conferma_da_hidden").datepicker( {
			minDate: new Date(annoDa, 0, 1),
			maxDate: new Date(annoA, 11, 31),
			yearRange: annoDa + ":" + annoA,
			showOn : "button",
			...
			buttonImageOnly : true,
			onSelect : function(dateText, inst) {
				$("#tx_data_conferma_da_day_id").val(dateText.substr(0, 2));
				$("#tx_data_conferma_da_month_id").val(dateText.substr(3, 2));
				$("#tx_data_conferma_da_year_id").val(dateText.substr(6, 4));
			},
			beforeShow: function(input, inst) {
				//imposta il valore del calendario in base a quanto impostato nelle 3 dropdownlist
	            updateValoreDatePickerFromDdl("tx_data_conferma_da_day_id",
				                            "tx_data_conferma_da_month_id",
				                            "tx_data_conferma_da_year_id",
				                            "tx_data_conferma_da_hidden");
			}
		});
		$("#tx_data_conferma_a_hidden").datepicker("option", "dateFormat", "dd/mm/yyyy");
		$("#tx_data_conferma_a_hidden").datepicker( {
			minDate: new Date(annoDa, 0, 1),
			maxDate: new Date(annoA, 11, 31),
			yearRange: annoDa + ":" + annoA,
			showOn : "button",
			...
			buttonImageOnly : true,
			onSelect : function(dateText, inst) {
				$("#tx_data_conferma_a_day_id").val(dateText.substr(0, 2));
				$("#tx_data_conferma_a_month_id").val(dateText.substr(3, 2));
				$("#tx_data_conferma_a_year_id").val(dateText.substr(6, 4));
			},
			beforeShow: function(input, inst) {
				//imposta il valore del calendario in base a quanto impostato nelle 3 dropdownlist
	            updateValoreDatePickerFromDdl("tx_data_conferma_a_day_id",
				                            "tx_data_conferma_a_month_id",
				                            "tx_data_conferma_a_year_id",
				                            "tx_data_conferma_a_hidden");
			}
		});
	});
			...
			<s:div name="divRicMetadati" cssclass="divRicMetadati">
				<s:div name="divRicMetadatiTop" cssclass="divRicMetadatiTop">
					<s:div name="divElement1" cssclass="divRicMetadatiTopLeft">
						...
						<s:dropdownlist name="ddlProvincia" disable="true"
					...
					<s:div name="divElement2" cssclass="divRicMetadatiCenterSmall">
						<s:dropdownlist name="ddlComune"
							...
							onchange="setFiredButton('tx_button_comune_changed');this.form.submit();"
							valueselected="${ddlComune}">
							<s:ddloption text="Tutti i Comuni" value="" />
							<s:ddloption value="{3}" text="{1} ({3})"/>
						</s:dropdownlist>
						<noscript><s:button id="tx_button_comuni_changed"
							onclick="" text="" type="submit"
							cssclass="btnimgStyle" title="Aggiorna" validate="false" /></noscript>
					</s:div>
				...
				<s:div name="divRicercaLeft" cssclass="divRicMetadatiLeft">
					...
					<s:div name="divElement13" cssclass="divRicMetadatiDoubleRow">
						<s:div name="divElement14a" cssclass="labelData">
							<s:label name="label_data"
								cssclass="seda-ui-label label85 bold textright" text="Data Conferma" />
						</s:div>

						<s:div name="divElement14" cssclass="floatleft">
							<s:div name="divDataConfermaDa" cssclass="divDataDa">
								<s:date label="Da:" prefix="tx_data_conferma_da"
									yearbegin="${ddlDateAnnoDa}" yearend="${ddlDateAnnoA}"
									locale="IT-it" descriptivemonth="false" separator="/"
									calendar="${tx_data_conferma_da}" cssclasslabel="labelsmall"
									cssclass="dateman">
								</s:date>
								<input type="hidden" id="tx_data_conferma_da_hidden" value="" />
							</s:div>

							<s:div name="divDataConfermaA" cssclass="divDataA">
								<s:date label="A:" prefix="tx_data_conferma_a"
									yearbegin="${ddlDateAnnoDa}" yearend="${ddlDateAnnoA}"
									locale="IT-it" descriptivemonth="false" separator="/"
									calendar="${tx_data_conferma_a}" cssclasslabel="labelsmall"
									cssclass="dateman">
								</s:date>
								<input type="hidden" id="tx_data_conferma_a_hidden" value="" />
							</s:div>
						</s:div>
					</s:div>
					...
					<s:div name="divElement6" cssclass="divRicMetadatiSingleRow">
						<s:dropdownlist name="txtTipoComunicazione" disable="false"
							...
							<s:ddloption text="Straordinaria" value="S" />
						</s:dropdownlist>
					</s:div>
	...
	<s:div name="divRisultati" cssclass="divRisultati">
		<s:datagrid cachedrowset="listaComunicazioni" action="monitoraggiowis.do?vista=monitoraggiowis" 
			...
			<s:dgcolumn label="Tipo Comunicaz." asc="TIPOA" desc="TIPOD" >
				...
				<s:if left="{12}" control="eq" right="I">
					<s:then>Integrativa</s:then>
				</s:if>
				<s:if left="{12}" control="eq" right="S">
					<s:then>Straordinaria</s:then>	
				</s:if>
			</s:dgcolumn>
			<s:dgcolumn index="26" label="N° Pernott." css="textright" asc="GGPAGA" desc="GGPAGD"/>
