...
<s:div name="divDettaglio" cssclass="divDettaglio">
	<s:table id="tableTestataComunicazione" border="1" cellspacing="0" cssclass="seda-ui-datagrid tableDettaglio1">
		...
		<s:tbody>
			...
			<s:tr>
				...
				<s:td>
					<c:choose>
						...
						<c:when test="${testatacomunicazione.tipoComunicazione == 'STRAORDINARIA'}">
							Straordinaria
						</c:when>
					</c:choose>
				</s:td>
			...
			<s:tr>
			    ...
			    <c:choose>
			    	<c:when test="${testatacomunicazione.modalitaPagamento == 'BOLLETTINO' && testatacomunicazione.statoComunicazione == 'CARICATA_SU_ENTRATE' && testatacomunicazione.numeroAvvisoPagoPA != ''}">
			    		Numero Avviso PagoPA:
			    	</c:when>
			    	<c:when test="${testatacomunicazione.modalitaPagamento == 'BOLLETTINO' && testatacomunicazione.statoComunicazione == 'CARICATA_SU_ENTRATE' && testatacomunicazione.numeroAvvisoPagoPA == ''}">
			    		Numero Bollettino Freccia:
			    	</c:when>
			    	<c:otherwise>
			    		Numero Bollettino:
			    	</c:otherwise>
				...
				<s:td>
			    <c:choose>
			    	<c:when test="${testatacomunicazione.numeroAvvisoPagoPA != ''}">
			    		${testatacomunicazione.numeroAvvisoPagoPA}
			    	</c:when>
			    	<c:otherwise>
			    		${testatacomunicazione.codiceBollettino}
			    	</c:otherwise>
			    </c:choose>
				</s:td>
				<!--  fine LP PG200230 -->
			...
			<s:tr>
				<s:td cssclass="seda-ui-cellheader">Codice RID:</s:td>
				<s:td>${testatacomunicazione.codiceRID}</s:td>
				...
				<s:td>
					<c:choose>
						<c:when test="${testatacomunicazione.modalitaPagamento == 'BOLLETTINO'}">
							<!--  inizio LP PG200230 -->
						    <c:choose>
						    	<c:when test="${testatacomunicazione.statoComunicazione == 'CARICATA_SU_ENTRATE' && testatacomunicazione.numeroAvvisoPagoPA != ''}">
									Bollettino PagoPA
						    	</c:when>
						    	<c:when test="${testatacomunicazione.statoComunicazione == 'CARICATA_SU_ENTRATE' && testatacomunicazione.numeroAvvisoPagoPA == ''}">
						    		Bollettino Freccia
			...
			<s:tr>
				<s:td cssclass="seda-ui-cellheader">Tariffa Giornaliera:</s:td>
				<s:td cssclass="text_align_right"><fmt:formatNumber type="NUMBER" value="${tariffacomunicazione.importoTariffa}" minFractionDigits="2" maxFractionDigits="2"/> </s:td>
				<s:td cssclass="seda-ui-cellheader">Username Operatore:</s:td>
				<s:td>${testatacomunicazione.usernameUtenteUltimoAggiornamento}</s:td>
			</s:tr>
			<s:tr>
				<s:td cssclass="seda-ui-cellheader">Data Conferma Comunicazione:</s:td>
				<s:td>${dataConfermaCom}</s:td>
	...
	<s:table id="tableAnagraficaStruttura" border="1" cellspacing="0" cssclass="seda-ui-datagrid tableDettaglioStruttura">
		...
		<s:tbody>
			...
			<s:tr>
				<s:td cssclass="seda-ui-cellheader">Flag Subentro:</s:td>
				<s:td>${anagraficastrutturacomunicazione.flagSubentro}</s:td>
				<s:td cssclass="seda-ui-cellheader"></s:td>
				<s:td></s:td>
			</s:tr>
			<s:tr>
				<s:td cssclass="seda-ui-cellheader">Data Validità inizio:</s:td>
				<s:td><s:formatDate pattern="dd/MM/yyyy" value="${anagraficastrutturacomunicazione.dataValiditaInizio}"/></s:td>
				<s:td cssclass="seda-ui-cellheader">Data Validità fine:</s:td>
				<s:td><s:formatDate pattern="dd/MM/yyyy" value="${anagraficastrutturacomunicazione.dataValiditaFine}"/></s:td>
			</s:tr>	
			<s:tr>
				<s:td cssclass="seda-ui-cellheader">Data Obbl.Comunicazione inizio:</s:td>
				<s:td><s:formatDate pattern="dd/MM/yyyy" value="${anagraficastrutturacomunicazione.dataObbligoComunicazioneInizio}"/></s:td>
				<s:td cssclass="seda-ui-cellheader">Data Obbl.Comunicazione fine:</s:td>
				<s:td><s:formatDate pattern="dd/MM/yyyy" value="${anagraficastrutturacomunicazione.dataObbligoComunicazioneFine}"/></s:td>
			</s:tr>	
		</s:tbody>
	</s:table>
	...
	<s:datagrid cssclass="dgDettaglioComunicazione" cachedrowset="listdettaglicomunicazione" action="" usexml="true" border="0">
		...
		<s:dgcolumn index="3" label="N° Pernottamenti" css="textright" />
