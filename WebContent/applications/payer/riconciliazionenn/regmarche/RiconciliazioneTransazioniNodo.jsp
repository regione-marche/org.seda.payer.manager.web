...
<m:view_state id="riconciliazionetransazioninodo" encodeAttributes="true" />
...
<script type="text/javascript">
	...
	$(function() {
		...
		//inizio LP PG200200
		$("#dtMakeFlusso_da_hidden").datepicker("option", "dateFormat", "dd/mm/yyyy"); 
		$("#dtMakeFlusso_da_hidden").datepicker(
						{
							minDate : new Date(annoDa, 0, 1),
							maxDate : new Date(annoA, 11, 31),
							yearRange : annoDa + ":" + annoA,
							...
							buttonImageOnly : true,
							onSelect : function(dateText, inst) {
								$("#dtMakeFlusso_da_day_id").val(dateText.substr(0, 2));
								$("#dtMakeFlusso_da_month_id").val(dateText.substr(3, 2));
								$("#dtMakeFlusso_da_year_id").val(dateText.substr(6, 4));
							},
							beforeShow : function(input, inst) {
								//imposta il valore del calendario in base a quanto impostato nelle 3 dropdownlist
							updateValoreDatePickerFromDdl(
									"dtMakeFlusso_da_day_id",
									"dtMakeFlusso_da_month_id",
									"dtMakeFlusso_da_year_id",
									"dtMakeFlusso_da_hidden");
						}
						});
		$("#dtMakeFlusso_a_hidden").datepicker("option", "dateFormat","dd/mm/yyyy");
		$("#dtMakeFlusso_a_hidden").datepicker(
						{
							minDate : new Date(annoDa, 0, 1),
							maxDate : new Date(annoA, 11, 31),
							yearRange : annoDa + ":" + annoA,
							...
							buttonImageOnly : true,
							onSelect : function(dateText, inst) {
								$("#dtMakeFlusso_a_day_id").val(dateText.substr(0, 2));
								$("#dtMakeFlusso_a_month_id").val(dateText.substr(3, 2));
								$("#dtMakeFlusso_a_year_id").val(dateText.substr(6, 4));
							},
							beforeShow : function(input, inst) {
								//imposta il valore del calendario in base a quanto impostato nelle 3 dropdownlist
							updateValoreDatePickerFromDdl(
									"dtMakeFlusso_a_day_id",
									"dtMakeFlusso_a_month_id",
									"dtMakeFlusso_a_year_id",
									"dtMakeFlusso_a_hidden");
						}
						});
		//fine LP PG200200
	});
...
<c:url value="" var="formParameters">
	...
	<%-- fine LP PG200200 --%>
	<%-- inizio LP PG200360 --%>
	<c:if test="${!empty param.recuperateFlusso}">
		<c:param name="recuperateFlusso">${param.recuperateFlusso}</c:param>
	</c:if>
	<%-- fine LP PG200360 --%>
	<c:if test="${!empty param.esitoInvioWS}">
					<c:param name="esitoInvioWS">${param.esitoInvioWS}</c:param>
	</c:if>
	<c:if test="${!empty ext}">
...
<%--
				...
				<s:div name="divRicercaLeft" cssclass="divRicMetadatiLeft">
					...
					<%-- inizio LP PG200200 --%>
					<s:div name="divElement122" cssclass="divRicMetadatiDoubleRow">
						<s:div name="divElement122a" cssclass="labelData">
							<s:label name="label_data_creazione2" 
								cssclass="seda-ui-label label78 bold textright"
								text="Data Creazione Movimento" />
						</s:div>

						<s:div name="divElement122b" cssclass="floatleft">
							<s:div name="divMkFlussoDA" cssclass="divDataDa">
								<s:date label="Da:" cssclasslabel="labelsmall"
									cssclass="dateman" prefix="dtMakeFlusso_da"
									yearbegin="${ddlDateAnnoDa}" yearend="${ddlDateAnnoA}"
									locale="IT-it" descriptivemonth="false" separator="/"
									calendar="${dtMakeFlusso_da}">
								</s:date>
								<input type="hidden" id="dtMakeFlusso_da_hidden" value="" />
							</s:div>

							<s:div name="divMkFlussoA" cssclass="divDataA">
								<s:date label="A:" cssclasslabel="labelsmall" cssclass="dateman"
									prefix="dtMakeFlusso_a" yearbegin="${ddlDateAnnoDa}"
									yearend="${ddlDateAnnoA}" locale="IT-it"
									descriptivemonth="false" separator="/"
									calendar="${dtMakeFlusso_a}"></s:date>
								<input type="hidden" id="dtMakeFlusso_a_hidden" value="" />
							</s:div>
						</s:div>
					</s:div>
					<%-- fine LP PG200200 --%>
				...
				<s:div name="divRicercaCenter" cssclass="divRicMetadatiCenter">
						...
						<s:div name="divElement10b" cssclass="floatleft">
							<s:div name="divImportoDa" cssclass="divDataDa">
								<s:textbox validator="ignore;numberIT;maxlength=15"
									maxlenght="15" bmodify="true" name="importo_movimento_da"
							...
							<s:div name="divImportoA" cssclass="divDataA">
								<s:textbox validator="ignore;numberIT;maxlength=15"
									maxlenght="15" bmodify="true" name="importo_movimento_a"
					...
					<s:div name="divElement10" cssclass="divRicMetadatiDoubleRow">
						<s:div name="divElement10a" cssclass="labelData">
							<s:label name="label_importo"
								cssclass="label85 bold floatleft textright" text="Importo" />
						</s:div>
						<s:div name="divElement10b" cssclass="floatleft">
							<s:div name="divImportoDa" cssclass="divDataDa">
								<s:textbox validator="ignore;numberIT;maxlength=15"
					...
					<s:div name="divElement14e" cssclass="divRicMetadatiSingleRow">
						<s:dropdownlist name="esitoInvioWS" cssclass="tbddl floatleft"
							cssclasslabel="label85 bold floatleft textright" disable="false"
							label="Invio WS:" valueselected="${esitoInvioWS}">
							<s:ddloption value="" text="Tutte" />
							<s:ddloption value="2" text="Notificate" />
...
<!-- 
 ...
 -->
				...
				<s:div name="divRicercaRight" cssclass="divRicMetadatiRight">
					...
					<%-- inizio LP PG200360 --%>
					<s:div name="divElement14e" cssclass="divRicMetadatiSingleRow">
						<s:dropdownlist name="recuperateFlusso" cssclass="tbddl floatleft"
							cssclasslabel="label85 bold floatleft textright" disable="false"
							label="Recuperate:" valueselected="${recuperateFlusso}">
							<s:ddloption value="" text="Tutte" />
							<s:ddloption value="Y" text="Si" />
							<s:ddloption value="N" text="No" />
						</s:dropdownlist>
...
<!-- lep utente eliminato -->
		...
		<%-- inizio LP PG200360 --%>
		<s:dgcolumn index="35" label="IUV recuperate" asc="NREA" desc="NRED" css="text_align_right"></s:dgcolumn>
		<%-- fine LP PG200360 --%>
		<s:dgcolumn index="1" label="Id. Quadratura" asc="KEYA" desc="KEYD" css="text_align_right"></s:dgcolumn>
			...
			<s:thendatagrid>
				...
				<%-- fine LP PG200200 --%>
				<%-- ATTENZIONE L'INDICE è ORA 36 SUL SERVER MYSQL  ( reg marche )  IN CASO DI MERGE SP DB2 MYSQL CALCOLARE IL GIUSTO INDEX E VALORIZZARLO ANCHE NEL LINK AL DETTAGLIO SOTTOSTANTE--%>
				<!-- YLM PG22XX05_26 -->
				<%-- <s:dgcolumn index="36" label="Errore" css="text_align_right"></s:dgcolumn> --%>
			</s:thendatagrid>
		</s:ifdatagrid>
		<s:dgcolumn label="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Azioni&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;">
			...
			<s:if right="{32}" control="eq" left="Y">
				<s:then>
			...
			<s:if right="{33}" control="eq" left="1">
				<s:then>
					<s:hyperlink href="reinvioFlussoQuadraturaNodo.do?FileNameFlusso={6}&idDominio={25}&cutecute={4}&idFlusso={17}&keyQuadratura={1}&keyEnte={34}&codSoc={3}"
					imagesrc="../applications/templates/riconciliazionenn/img/icon_invio_red.png" 
					alt="Ritenta invio Flusso" text="" cssclass="hlStyle" />
				</s:then>
				<s:else>
				</s:else>
			</s:if>
			<s:if right="{33}" control="eq" left="2">
				<s:then>
					<s:image src="../applications/templates/riconciliazionenn/img/icon_invio_green.png" alt="Flusso inviato" width="" height="" cssclass="hlStyle"/>
				</s:then>
				<s:else>
				</s:else>
			</s:if>
