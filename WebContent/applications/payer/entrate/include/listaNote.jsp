...
<c:if test="${!empty dettaglioTributo}">
	<fmt:setLocale value="it_IT"/>
	...
	<s:div name="divDettTributo" cssclass="divRisultati">

	<s:datagrid  viewstate="" cachedrowset="dettaglioTributo" action="nessuna.do" border="1" usexml="true"  rowperpage="${applicationScope.rowsPerPage}">
		<s:action>
			<c:url value="nessuna.do">				
			</c:url>
		</s:action>
  	    <!-- EH1_CISECISE -->
		<s:dgcolumn label=" " index="1" ></s:dgcolumn>
  	    <!-- EH1_CISECISE -->
		<s:dgcolumn label="Tributo" index="2" ></s:dgcolumn>

  	    <!-- EH1_CEH1ADOC -->
		<s:dgcolumn index="3" label="Anno Riferimento"  ></s:dgcolumn>

 		<!-- CARICO -->
		<s:dgcolumn index="4" format="#,##0.00" label="Carico" css="text_align_right"></s:dgcolumn>

		<!-- RISCOSSO -->
		<s:dgcolumn index="5" format="#,##0.00" label="Riscosso" css="text_align_right"></s:dgcolumn>
        
		<!-- RESIDUO -->
		<s:dgcolumn index="6" format="#,##0.00" label="Residuo" css="text_align_right"></s:dgcolumn>
		
	</s:datagrid>
 </s:div>
	</c:if>
 	
<c:if test="${!empty listaNote}">
	<fmt:setLocale value="it_IT"/>

	<s:div name="divTableTitle5" cssclass="divTableTitle bold">
		Lista Note
	</s:div>

	<s:div name="divRisultati" cssclass="divRisultati">

	<s:datagrid  viewstate="" cachedrowset="listaNote" action="ricercaNote.do" border="1" usexml="true"  rowperpage="${applicationScope.rowsPerPage}">
		<s:action>
			<c:url value="ricercaNote.do">
		...
		<c:if test="${!empty ext}">
			<c:param name="ext">${ext}</c:param>
				...
				<!--  pag 3  -->
				<c:if test="${!empty r3RowsPerPage}">
					<c:param name="r3RowsPerPage">${r3RowsPerPage}</c:param> 
				</c:if>
				<c:if test="${!empty r3PageNumber}">
					<c:param name="r3PageNumber">${r3PageNumber}</c:param> 
				</c:if>
				<c:if test="${!empty r3Order}">
					<c:param name="r3Order">${r3Order}</c:param> 
				</c:if>
				<c:if test="${!empty r3Ext}">
					<c:param name="r3Ext">${r3Ext}</c:param>
				</c:if>
				...
				<c:if test="${!empty chiaveCodTrib}">
					<c:param name="chiaveCodTrib">${chiaveCodTrib}</c:param> 
				</c:if>
				<c:if test="${!empty chiaveAnnoTrib}">
					<c:param name="chiaveAnnoTrib">${chiaveAnnoTrib}</c:param> 
				</c:if>				
				<c:if test="${!empty chiaveProgTrib}">
					<c:param name="chiaveProgTrib">${chiaveProgTrib}</c:param> 
				</c:if>				
				<c:if test="${!empty chiaveFlTrib}">
					<c:param name="chiaveFlTrib">${chiaveFlTrib}</c:param> 
				</c:if>				
				<c:if test="${!empty chiaveSrTrib}">
					<c:param name="chiaveSrTrib">${chiaveSrTrib}</c:param> 
				</c:if>				
				<c:if test="${!empty chiaveTbTrib}">
					<c:param name="chiaveTbTrib">${chiaveTbTrib}</c:param> 
				</c:if>	
				<!--  
				<c:if test="${!empty ext}">
					<c:param name="ext">${ext}</c:param>
  	    ...
  	    <!-- NOTE -->
		<s:dgcolumn index="8" label="Descrizione Nota"></s:dgcolumn>
	...
	<s:div name="indietro" cssclass="divRicBottoni">
		<s:form name="ricercaNoteForm" action="ricercaTributi.do" method="post" hasbtn1="false" hasbtn2="false" hasbtn3="false">
			...
			<s:textbox name="r2RowsPerPage" label="rowsPerPageRic2" bmodify="true" text="${r2RowsPerPage}" cssclass="display_none" cssclasslabel="display_none"  />
			<s:textbox name="r2PageNumber" label="pageNumberRic2" bmodify="true" text="${r2PageNumber}" cssclass="display_none" cssclasslabel="display_none"  />
			<s:textbox name="r2Order" label="orderRic2" bmodify="true" text="${r2Order}" cssclass="display_none" cssclasslabel="display_none"  />
			<s:textbox name="r2Ext" label="r2Ext" bmodify="true" text="${r2Ext}" cssclass="display_none" cssclasslabel="display_none"  />
	
			<s:textbox name="rowsPerPage" label="rowsPerPageRic3" bmodify="true" text="${r3RowsPerPage}" cssclass="display_none" cssclasslabel="display_none"  />
			<s:textbox name="pageNumber" label="pageNumberRic3" bmodify="true" text="${r3PageNumber}" cssclass="display_none" cssclasslabel="display_none"  />
			<s:textbox name="order" label="orderRic3" bmodify="true" text="${r3Order}" cssclass="display_none" cssclasslabel="display_none"  />
			<s:textbox name="ext" label="ext" bmodify="true" text="${r3Ext}" cssclass="display_none" cssclasslabel="display_none"  />
			...
			<s:div name="divRicercaBottoni" cssclass="divRicBottoni">
				<s:button id="tx_button_cerca_exp" cssclass="btnStyle" type="submit" text="Indietro" onclick="" />
