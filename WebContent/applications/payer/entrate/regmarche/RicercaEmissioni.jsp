...
<script type="text/javascript">
	...
	<c:url value="" var="formParameters2">
		<c:param name="testa2">ok</c:param>
		<c:if test="${!empty rowsPerPage}">
			<c:param name="rRowsPerPage">${rowsPerPage}</c:param> 
		</c:if>
		<c:if test="${!empty pageNumber}">
...
<s:div name="div_selezione" cssclass="div_align_center divSelezione">
	<s:div name="divRicercaTopName" cssclass="divRicercaTop">
		...
		<s:form name="ricercaEmissioniForm" action="ricercaEmissioni.do" method="post" hasbtn1="false" hasbtn2="false" hasbtn3="false">
	...
	-->
				...
				<s:div name="divRicMetadati" cssclass="divRicMetadati">
					...
					<s:div name="divRicercaLeft" cssclass="divRicMetadatiLeft">
						<s:div name="divElement4" cssclass="divRicMetadatiSingleRow">
							<s:dropdownlist name="impostaServ"
								...
								<s:ddloption text="Tutte le Imposte Servizio" value="" />
					...
					<s:div name="divRicercaLeft2" cssclass="divRicMetadatiRight">
...
<c:if test="${!empty listaEmissioni}">
	...
	<s:div name="divRisultati" cssclass="divRisultati">
		...
		<s:datagrid  viewstate="" cachedrowset="listaEmissioni" action="ricercaEmissioni.do" border="1" usexml="true"  rowperpage="${applicationScope.rowsPerPage}">
			<s:action>
				<c:url value="ricercaEmissioni.do" >
					<c:if test="${!empty tx_societa}">  
						<c:param name="tx_societa">${tx_societa}</c:param>
					</c:if>
					<c:if test="${!empty tx_utente}">  
	...
	<!-- 
	 ...
	 -->
			...
			<!-- SOCIETA -->
			<s:dgcolumn index="15"  label="Societ&agrave;" asc="SOCA" desc="SOCD"></s:dgcolumn>
			
			<!-- UTENTE -->
			<s:dgcolumn index="7"  label="Utente" asc="UTEA" desc="UTED"></s:dgcolumn>
	
			<!-- ENTE -->
			<s:dgcolumn index="8"  label="Ente" asc="ENTA" desc="ENTD"></s:dgcolumn>
			...
			<s:dgcolumn  label="Ufficio" asc="UFFA" desc="UFFD">
				<s:if left="{4}{5}" control="eq" right="">
						<s:then>
						</s:then>
						<s:else>
							{4}/{5}
			...
			<!-- EH1_CTSECTSE -->
			<s:dgcolumn index="6" label="Tip. Ser." asc="CTSA" desc="CTSD" css="text_align_right"></s:dgcolumn>
	
			<!-- CARICO -->
			<s:dgcolumn index="9" format="#,##0.00" label="Carico" asc="CARA" desc="CARD" css="text_align_right"></s:dgcolumn>
	
			<!-- DIM. CARICO -->
			<s:dgcolumn index="11" format="#,##0.00" label="Dim.Carico" asc="DCAA" desc="DCAD" css="text_align_right"></s:dgcolumn>
	
			<!-- RISCOSSO -->
			<s:dgcolumn index="10" format="#,##0.00" label="Riscosso" asc="RSCA" desc="RSCD" css="text_align_right"></s:dgcolumn>
	        
			<!-- RIMBORSO -->
			<s:dgcolumn index="12" format="#,##0.00" label="Rimborso" asc="RIMA" desc="RIMD" css="text_align_right"></s:dgcolumn>
	        
			<!-- RENDICONTATO  -->
			<s:dgcolumn index="13" format="#,##0.00" label="Rendicont." asc="RNDA" desc="RNDD" css="text_align_right"></s:dgcolumn>
	
			<!-- RESIDUO -->
			<s:dgcolumn index="14" format="#,##0.00" label="Residuo" asc="RSDA" desc="RSDD" css="text_align_right"></s:dgcolumn>

			<!-- RESIDUO SCAD -->
			<s:dgcolumn index="17" format="#,##0.00" label="Residuo Scad." asc="RSCAA" desc="RSCAD" css="text_align_right"></s:dgcolumn>
	
	        <s:ifdatagrid left="${ext}" control="eq" right="1">
			</s:ifdatagrid>
	
		    <s:dgcolumn label="Dett." >
				<s:hyperlink href="dettaglioEmissione.do${formParameters}&tx_button_cerca=OK&chiaveCodUte={7}&chiaveCodEnte={8}&chiaveTipoUff={4}&chiaveCodUff={5}&chiaveIS={1}&chiaveAnnoE={2}&chiaveNumeroE={3}"
				 imagesrc="../applications/templates/shared/img/dettaglio.png"
				 text="" 
				 alt="Dettaglio Emissione"
				 cssclass="blacklink hlStyle" />
	        </s:dgcolumn>
		    <s:dgcolumn label="Doc." >
				{16}&nbsp;
				<s:hyperlink href="ricercaDocumentiEmi.do${formParameters2}&tx_button_cerca=OK&chiaveCodUte={7}&chiaveCodEnte={8}&chiaveTipoUff={4}&chiaveCodUff={5}&chiaveIS={1}&chiaveAnnoE={2}&chiaveNumeroE={3}"
		...
		<s:table  border="1" cellspacing="0" cellpadding="3"  cssclass="seda-ui-datagrid">
			<s:thead>
				<s:tr cssclass="seda-ui-datagridrowpari">
					<s:th cssclass="seda-ui-datagridheadercell"  icol="8"><b>Totali Emissioni</b></s:th>
				</s:tr>
			</s:thead>
			<s:tbody>
				<s:tr cssclass="seda-ui-datagridrowpari">
