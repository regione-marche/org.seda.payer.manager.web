...
<script type="text/javascript" >
	...
	<c:url value="" var="formParameters">
	 ...
	 <s:div name="div_selezione" cssclass="div_align_center divSelezione">
			<s:form name="ricercaPartiteRuoliForm" action="ricercaRiversamenti.do" method="post" hasbtn1="false" hasbtn2="false" hasbtn3="false">
			</s:form>
	</s:div>
...
<c:if test="${!empty listaPartite}">
	...
	<s:datagrid  viewstate="" cachedrowset="listaPartite" action="ricercaPartiteRuoli.do" border="1" usexml="true"  rowperpage="${applicationScope.rowsPerPage}">
		<s:action>
			<c:url value="ricercaPartiteRuoli.do" >
		<c:if test="${!empty rRowsPerPage}">
			<c:param name="rRowsPerPage">${rRowsPerPage}</c:param> 
		</c:if>
		<c:if test="${!empty rPageNumber}">
		...
		<!-- RESIDUO asc="RSDA" desc="RSDD"-->
		<s:dgcolumn index="17" format="#,##0.00" label="Residuo" asc="RSDA" desc="RSDD" css="text_align_right"></s:dgcolumn>
	    ...
	    <s:dgcolumn label="Dett." >
	    	&nbsp;
			<s:hyperlink href="dettaglioPartita.do${formParameters}&tx_button_cerca_exp=OK&chiaveNumPar={9}&chiaveConcPar={20}&chiaveTombPar={21}&r2RowsPerPage=${param.rowsPerPage}&r2PageNumber=${param.pageNumber}&r2Order=${param.order}&pagPrec=ricercaRuoli" 
	    ...
	    <s:dgcolumn label="Art." >
			{18}&nbsp;
			<s:hyperlink href="ricercaArticoli.do${formParameters}&tx_button_cerca_exp=OK&chiaveNumPar={9}&chiaveConcPar={20}&chiaveTombPar={21}&r2RowsPerPage=${param.rowsPerPage}&r2PageNumber=${param.pageNumber}&r2Order=${param.order}&pagPrec=ricercaRuoli" 
			alt="Lista Articoli" 
	    ...
	    <s:dgcolumn label="Pag." >
			...
			<s:else>
			{19}&nbsp;
			<s:hyperlink href="ricercaPagamentiDett.do${formParameters}&tx_button_cerca_exp=OK&chiaveNumPar={9}&chiaveConcPar={20}&chiaveTombPar={21}&r2RowsPerPage=${param.rowsPerPage}&r2PageNumber=${param.pageNumber}&r2Order=${param.order}&pagPrec=ricercaRuoli" 
			alt="Lista Pagamenti" 
			imagesrc="../applications/templates/shared/img/listaDati.png"
			text="" 
			cssclass="blacklink hlStyle" />
...
<s:div name="divDatiAgg" cssclass="divTableTitle bold text_align_right">
	...
	<s:div name="indietro" cssclass="divRicBottoni">
		<s:form name="ricercaRuoli" action="ricercaRuoli.do" method="post" hasbtn1="false" hasbtn2="false" hasbtn3="false">
