...
<!-- onchange="setDdl('provincia'); callSubmit(this.form);" -->
				...
				<s:div name="divRicercaLeft" cssclass="divRicMetadatiLeft">
					...
					<s:div name="divElement5" cssclass="divRicMetadatiSingleRow">
						<s:textbox 	validator="maxlength=16"
									maxlenght="16"
								 	cssclass="textareaman"
									cssclasslabel="label85 bold textright"
									bmodify="true" name="cro" label="CRO:" text="${cro}"/>
					</s:div>
			...
			<s:div name="divCentered0" cssclass="divRicBottoni">
				...
				<s:button id="tx_button_reset" type="submit" text="Reset" onclick="" cssclass="btnStyle" />
			 	<s:button id="tx_button_esportadati" type="submit" text="Esporta Dati" onclick="" cssclass="btnStyle" disable="${!esportaDatiEnabled}"/>
...
<c:if test="${!empty listaMovimenti}">
	...
	<s:datagrid  viewstate="" cachedrowset="listaMovimenti" action="riconciliazioneTransazioni.do" border="1" usexml="true"  rowperpage="${applicationScope.rowsPerPage}">
		<s:action>
			<c:url value="riconciliazioneTransazioni.do">
					...
					<c:if test="${!empty param.data_valuta_a_day}">  
						<c:param name="data_valuta_a_day">${param.data_valuta_a_day}</c:param>
					</c:if>
					<c:if test="${!empty param.data_valuta_a_month}">  
						<c:param name="data_valuta_a_month">${param.data_valuta_a_month}</c:param>
					</c:if>
		...
		<s:dgcolumn label="Utente" index="2" asc="UTEA" desc="UTED"></s:dgcolumn>
		...
		<s:dgcolumn index="9" label="CRO" asc="CROA" desc="CROD"></s:dgcolumn>
		...
		<s:dgcolumn index="13" label="Periodo  (da)" format="dd/MM/yyyy HH:mm:ss" asc="DINA" desc="DIND"></s:dgcolumn>
		<s:dgcolumn index="14" label="Periodo  (a)" format="dd/MM/yyyy HH:mm:ss" asc="DFIA" desc="DFID"></s:dgcolumn>
			...
			<s:if right="{18}" control="eq" left="A">
				<s:then>
					<c:if test="${sessionScope.j_user_bean.azioniPerRiconciliazioneManualeEnabled}">
						<s:hyperlink href="riconciliazioneManualeTransazioni.do?chiaveMovimento={1}&codiceAbi={4}&codiceSia={3}&codiceCab={5}&ccb={6}&nomeSupporto={8}&cro={9}&data_pagamento_daS={13}&data_pagamento_aS={14}&tx_societa={20}&tx_utente={21}&tx_provincia=${tx_provincia}&data_creazioneS={7}&tx_tipo_carta={22}&tx_button_movimento_changed=Selezione"
	...
	<!--<s:table border="0" cellspacing="0" cellpadding="3" cssclass="seda-ui-datagrid">
		<s:thead>
			<s:tr>
				<s:th cssclass="seda-ui-datagridheadercell"><b>Riepilogo statistico</b></s:th>
			</s:tr>
		</s:thead>
		<s:tbody><s:tr><s:td>&nbsp;</s:td></s:tr></s:tbody>
	</s:table>-->
	...
	<s:table  border="1" cellspacing="0" cellpadding="3"  cssclass="seda-ui-datagrid">
		<s:thead>
			<s:tr cssclass="seda-ui-datagridrowpari">
				<s:th cssclass="seda-ui-datagridheadercell"  icol="7"><b>Importi Totali sui rispettivi Gateway</b></s:th>
			</s:tr>
		</s:thead>
		<s:tbody>
			<s:tr cssclass="seda-ui-datagridrowdispari">
				<s:td cssclass="seda-ui-datagridcell">&nbsp;<br/>Tipo Carta</s:td>
			...
			<c:forEach items="${requestScope.listaMovimentiGrouped}" var="quadratura">
				<s:tr cssclass="seda-ui-datagridrowpari">
					<s:td cssclass="seda-ui-datagridcell"><c:out value="${quadratura.carta}"/></s:td>
					<s:td cssclass="seda-ui-datagridcell text_align_right"><c:out value="${quadratura.numeroMovimenti}"/></s:td>
					<s:td cssclass="seda-ui-datagridcell text_align_right"><fmt:formatNumber type="NUMBER" value="${quadratura.importoMovimenti}" minFractionDigits="2" maxFractionDigits="2"/></s:td>
					<s:td cssclass="seda-ui-datagridcell text_align_right"><c:out value="${quadratura.numeroTransazioni}"/></s:td>
				</s:tr>
			</c:forEach>
			<s:tr cssclass="seda-ui-datagridrowdispari">
