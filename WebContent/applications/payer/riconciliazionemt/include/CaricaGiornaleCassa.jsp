...
$(document).ready( function() {
    if(document.getElementById("uploadGiornale").files.length == 0) {
...
<c:url value="" var="formParameters">
	...
	<c:if test="${!empty param.uploadGiornale}">
		<c:param name="uploadGiornale">${param.uploadGiornale}</c:param>
	</c:if>
</c:url>
...
<s:div name="divSelezione1" cssclass="divSelezione">
	<s:div name="divRicercaTopName" cssclass="divRicercaTop">
		<c:if test="${requestScope.upload != null}">
			<div class="seda-ui-divvalidator"><span class="seda-ui-spanvalidator">${requestScope.message}<br></span></div>
		</c:if>
		<s:form name="caricaGiornaleCassaForm"
			action="caricaGiornaleCassa.do" method="post"
		...
		<s:form name="uploadForm"
			action="upload.do${formParameters}" method="post"
				...
				<s:div name="divRicercaLeft" cssclass="divRicMetadatiLeft">
					<s:div name="divElement14" cssclass="divRicMetadatiDoubleRow">
						<s:table border="">
							...
							<s:tbody>
								<s:tr>
									<s:td><s:label name="" text="Giornale di Cassa:" cssclass="label65 bold textright"/></s:td>
									<s:td><input class="file-input" type="file" name="uploadGiornale" id="uploadGiornale" onchange="enableBtn()"/></s:td>
								</s:tr>														
							</s:tbody>
						</s:table>	
					</s:div>										
			...
			<s:div name="divCentered0" cssclass="divRicBottoni">
				<s:hyperlink name="hpAnnulla" cssclass="hlToBtnStyle" href="ricercaGiornaliCassa.do${formParameters}" text="Indietro" />
				<!-- <s:hyperlink name="hpAnnulla" cssclass="aAnnulla" href="ricercaGiornaliCassa.do" text="Indietro" />  -->
				<s:button id="tx_button_upload" type="submit" text="Carica" onclick=""
					cssclass="btnStyle" />			
