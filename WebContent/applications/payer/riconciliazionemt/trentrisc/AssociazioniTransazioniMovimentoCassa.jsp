...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#dtTransazione_da_hidden").datepicker("option", "dateFormat","dd/mm/yyyy");
		$("#dtTransazione_da_hidden").datepicker(
			{
				minDate : new Date(annoDa, 0, 1),
				maxDate : new Date(annoA, 11, 31),
				yearRange : annoDa + ":" + annoA,
				...
				buttonImageOnly : true,
				onSelect : function(dateText, inst) {
					$("#dtTransazione_da_day_id").val(dateText.substr(0, 2));
					$("#dtTransazione_da_month_id").val(dateText.substr(3, 2));
					$("#dtTransazione_da_year_id").val(dateText.substr(6, 4));
				},
				beforeShow : function(input, inst) {
					//imposta il valore del calendario in base a quanto impostato nelle 3 dropdownlist
				updateValoreDatePickerFromDdl(
						"dtTransazione_da_day_id",
						"dtTransazione_da_month_id",
						"dtTransazione_da_year_id",
						"dtTransazione_da_hidden");
			}
		});
		$("#dtTransazione_a_hidden").datepicker("option", "dateFormat","dd/mm/yyyy");
		$("#dtTransazione_a_hidden").datepicker(
			{
				minDate : new Date(annoDa, 0, 1),
				maxDate : new Date(annoA, 11, 31),
				yearRange : annoDa + ":" + annoA,
				...
				buttonImageOnly : true,
				onSelect : function(dateText, inst) {
					$("#dtTransazione_a_day_id").val(dateText.substr(0, 2));
					$("#dtTransazione_a_month_id").val(dateText.substr(3, 2));
					$("#dtTransazione_a_year_id").val(dateText.substr(6, 4));
				},
				beforeShow : function(input, inst) {
					//imposta il valore del calendario in base a quanto impostato nelle 3 dropdownlist
				updateValoreDatePickerFromDdl(
						"dtTransazione_a_day_id",
						"dtTransazione_a_month_id",
						"dtTransazione_a_year_id",
						"dtTransazione_a_hidden");
			}
		});
...
<c:url value="" var="formParameters">
	...
	<c:if test="${!empty param.dtTransazione_da_day}">
		<c:param name="dtTransazione_da_day">${param.dtTransazione_da_day}</c:param>
	</c:if>
	<c:if test="${!empty param.dtTransazione_da_month}">
		<c:param name="dtTransazione_da_month">${param.dtTransazione_da_month}</c:param>
	</c:if>
	<c:if test="${!empty param.dtTransazione_da_year}">
		<c:param name="dtTransazione_da_year">${param.dtTransazione_da_year}</c:param>
	</c:if>
	<c:if test="${!empty param.dtTransazione_a_day}">
		<c:param name="dtTransazione_a_day">${param.dtTransazione_a_day}</c:param>
	</c:if>
	<c:if test="${!empty param.dtTransazione_a_month}">
		<c:param name="dtTransazione_a_month">${param.dtTransazione_a_month}</c:param>
	</c:if>
	<c:if test="${!empty param.dtTransazione_a_year}">
		<c:param name="dtTransazione_a_year">${param.dtTransazione_a_year}</c:param>
	</c:if>
	<c:if test="${!empty param.idTransazione}">
		<c:param name="idTransazione">${param.idTransazione}</c:param>
	</c:if>
	<c:if test="${!empty param.idIUV}">
		<c:param name="idIUV">${param.idIUV}</c:param>
	</c:if>
	<c:if test="${!empty param.idIUR}">
		<c:param name="idIUR">${param.idIUR}</c:param>
	</c:if>
...
<s:div name="divSelezione1" cssclass="divSelezione">
	<s:div name="divRicercaTopName" cssclass="divRicercaTop">
		<s:form name="associazioniTransazioniMovimentoCassaForm"
			action="associazioniTransazioniMovimentoCassa.do" method="post"
			...
			<s:div name="divRicercaTitleName" cssclass="divRicTitle bold">
				Associazioni Transazioni A Movimento
			</s:div>
				...
				<s:div name="divRicercaCenter" cssclass="divRicMetadatiCenter">
					...
					<s:div name="divElement8" cssclass="divRicMetadatiSingleRow">
						<s:dropdownlist name="idcassa" cssclass="tbddl floatleft"
							...
							label="Identificativo Flusso:" valueselected="${idcassa}"
				...
				<s:div name="divRicercaRight" cssclass="divRicMetadatiRight">
					...
					<s:div name="divElement8" cssclass="divRicMetadatiSingleRow">
						<s:dropdownlist name="numeroDocumento" cssclass="tbddl floatleft"
							...
							label="Numero Movimento:" valueselected="${numeroDocumento}"
			...
			<s:div name="divCentered0" cssclass="divRicBottoni">
				...
				<s:table cssclass="seda-ui-datagrid" border="1" cellspacing="0" cellpadding="3">
					<s:thead>
						<s:tr cssclass="seda-ui-datagridrowdispari">
							<s:th cssclass="seda-ui-datagridcell">Importo Movimento</s:th>
				...
				<s:div name="divRicercaLeft" cssclass="divRicMetadatiLeft">
					<s:div name="divElement12" cssclass="divRicMetadatiDoubleRow">
						<s:div name="divElement12a" cssclass="labelData">
							<s:label name="label_data_creazione" 
								cssclass="seda-ui-label label78 bold textright"
								text="Data Transazioni" />
						</s:div>

						<s:div name="divElement12b" cssclass="floatleft">
							<s:div name="divDtTransazioneDA" cssclass="divDataDa">
								<s:date label="Da:" cssclasslabel="labelsmall"
									cssclass="dateman" prefix="dtTransazione_da"
									yearbegin="${ddlDateAnnoDa}" yearend="${ddlDateAnnoA}"
									locale="IT-it" descriptivemonth="false" separator="/"
									calendar="${dtTransazione_da}">
								</s:date>
								<input type="hidden" id="dtTransazione_da_hidden" value="" />
							</s:div>

							<s:div name="divDtTransazioneA" cssclass="divDataA">
								<s:date label="A:" cssclasslabel="labelsmall" cssclass="dateman"
									prefix="dtTransazione_a" yearbegin="${ddlDateAnnoDa}"
									yearend="${ddlDateAnnoA}" locale="IT-it"
									descriptivemonth="false" separator="/"
									calendar="${dtTransazione_a}"></s:date>
								<input type="hidden" id="dtTransazione_a_hidden" value="" />
							</s:div>
						</s:div>
					</s:div>
				...
				<s:div name="divRicercaCenter" cssclass="divRicMetadatiCenter">
					...
					<s:div name="divElement14" cssclass="divRicMetadatiSingleRow">
						<s:textbox validator="maxlength=36" maxlenght="36"
							cssclass="textareaman" cssclasslabel="label85 bold textright"
							bmodify="true" name="idTransazione" label="Identificativo Transazione:"
							text="${idTranszione}"/>
					</s:div>
					...
					<s:div name="divElement14" cssclass="divRicMetadatiSingleRow">
						<s:textbox validator="" maxlenght="35"
							cssclass="textareaman" cssclasslabel="label85 bold textright"
							bmodify="true" name="idIUR" label="Identificativo IUR:"
							text="${idIUR}" />
					</s:div>
				...
				<s:div name="divRicercaRight" cssclass="divRicMetadatiRight">
					...
					<s:div name="divElement14" cssclass="divRicMetadatiSingleRow">
						<s:textbox validator="maxlength=35" maxlenght="35"
							cssclass="textareaman" cssclasslabel="label85 bold textright"
							bmodify="true" name="idIUV" label="Identificativo IUV:"
							text="${idIUV}" />
					</s:div>
...
<c:if test="${!empty lista_transazioni}">
	<fmt:setLocale value="it_IT" />
	...
	<s:datagrid cachedrowset="lista_transazioni"
		action="associazioniTransazioniMovimentoCassa.do${formParameters}&page=${page}" border="1" usexml="true"
		rowperpage="${applicationScope.rowsPerPage}" viewstate="true">

		<s:dgcolumn index="2" label="Data Transazione" format="dd/MM/yyyy" asc="DTRA" desc="DTRD"></s:dgcolumn>
		<s:dgcolumn index="1" label="Id. Transazione" asc="IDTA" desc="IDTD"></s:dgcolumn>
		<s:dgcolumn index="3" label="IUV" asc="IUVA" desc="IUVD"></s:dgcolumn>
		<s:dgcolumn index="4" label="IUR" asc="IURA" desc="IURD"></s:dgcolumn>
		<s:dgcolumn index="5" label="Importo" css="text_align_right" format="#,##0.00" asc="IMPA" desc="IMPD"></s:dgcolumn>
		<s:dgcolumn label="Azioni">
		<s:if right="{6}" control="eq" left="0">
				<s:then>
					<c:if test="${sessionScope.j_user_bean.associazioniProvvisorieRiconciliazionemtEnabled}">
						<s:hyperlink
							href="associazioniTransazioniMovimentoCassa.do?numeroDocumento=${numeroDocumento}&chiaveTransazione={7}&action=aggiungi&page=${page}"
							cssclass="blacklink hlStyle"
							imagesrc="../applications/templates/riconciliazionemt/img/aggiungi.png" text=""
							alt="Associa al Movimento"
							onclick=""/>
					</c:if>
				</s:then>
				<s:else>
					<c:if test="${sessionScope.j_user_bean.associazioniProvvisorieRiconciliazionemtEnabled}">
						<s:hyperlink
							href="associazioniTransazioniMovimentoCassa.do?numeroDocumento=${numeroDocumento}&chiaveTransazione={7}&action=elimina&page=${page}"
							cssclass="blacklink hlStyle"
							imagesrc="../applications/templates/riconciliazionemt/img/elimina.png" text=""
							alt="Disassocia dal Movimento"
							onclick=""/>
					</c:if>
				</s:else>
			</s:if>
