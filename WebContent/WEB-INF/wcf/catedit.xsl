...
<xsl:template match="cat-category">
  <tr>
    <th align="left" class="navi-axis">
      <img src="{$context}/{$payerpath}/wcf/catedit/{@icon}" width="9" height="9"/>
      <xsl:text> </xsl:text>
...
<xsl:template match="cat-button[@icon]">
  <input border="0" type="image" src="{$context}/{$payerpath}/wcf/catedit/{@icon}" name="{@id}" width="9" height="9"/>
  <xsl:text> </xsl:text>
</xsl:template>

<xsl:template match="cat-button">
  <img src="{$context}/{$payerpath}/wcf/catedit/empty.png" width="9" height="9"/>
  <xsl:text> </xsl:text>
</xsl:template>
