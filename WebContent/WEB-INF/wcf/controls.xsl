...
<xsl:template mode="ctrl" match="imgButton[@disabled='true']">
  <img src="{$context}/{$payerpath}/{@src}" title="{@label}" alt="{@label}"/>
</xsl:template>

<xsl:template mode="ctrl" match="imgButton">
  <input type="image" src="{$context}/{$payerpath}/{@src}" name="{@id}" title="{@label}" alt="{@label}">
    <xsl:call-template name="stdattrs"/>
  </input>
</xsl:template>
    ...
    <xsl:when test="@calendar">
      <table>
        <tr>
          ...
          <td>
            <img src="{$context}/{$payerpath}/{@src}" onclick="jscalendarPopUpCalendar(this,document.getElementById('{@id}'), '{@calendar}')"/>  
          </td>
        </tr>
      </table>
    </xsl:when>
