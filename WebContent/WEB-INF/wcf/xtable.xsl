          ...
          <tr>
            <xsl:if test="@editId">
              <th align="left" class="xtable-title">
                <input type="image" src="{$context}/{$payerpath}/wcf/table/edit.png" name="{@editId}" width="16" height="16"/>
              </th>
            </xsl:if>
            <xsl:if test="@title">
...
<xsl:template match="xth[@selectId]">
  <th>
    ...
    <xsl:apply-templates select="@colspan"/>
    <input type="image" border="0" name="{@selectId}" src="{$context}/{$payerpath}/wcf/table/select.png" width="16" height="16"/>
    <xsl:apply-templates/>
  </th>
</xsl:template>
...
<xsl:template match="xth[@sort]">
  <th>
    ...
    <input type="image" border="0" name="{@id}" src="{$context}/{$payerpath}/wcf/table/sort-{@sort}.png" width="9" height="9"/>
    <xsl:text> </xsl:text>
    <xsl:apply-templates/>
  </th>
</xsl:template>
...
<xsl:template match="xpagenav">
  <input type="image" border="0" name="{@id}" src="{$context}/{$payerpath}/wcf/table/page-{@direction}.png" width="16" height="16"/>
</xsl:template>
...
<xsl:template match="xgotopage">
  ...
  <xsl:text> </xsl:text>
  <input type="image" border="0" name="{@buttonId}" src="{$context}/{$payerpath}/wcf/table/gotopage.png" width="16" height="16"/>
</xsl:template>
