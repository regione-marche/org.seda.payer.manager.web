...
<xsl:variable name="overrideFormat" select="1"/>
...
<xsl:template match="drill-other">
  <!--<img src="{$context}/{$payerpath}/{$imgpath}/{@img}.gif" border="0" width="9" height="9"/>-->
</xsl:template>
  ...
  <xsl:if test = "not(@value='&#160;')">
        <xsl:if test = "@mso-number-format">
            ...
            <xsl:attribute name="style">
                <xsl:choose>
                    <xsl:when test="$overrideFormat = 1 and (@mso-number-format = '\#\,\#\#0.###' or @mso-number-format = '\#\,\#\#0')">
                        <xsl:text>mso-number-format:\#\,\#\#0.00</xsl:text>
                    </xsl:when>
                    <xsl:otherwise>
